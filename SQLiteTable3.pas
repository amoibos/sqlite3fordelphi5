...
//==============  compat layer for Delphi 5 =================================
function UTF8ToANSI(const pSrc: String): String;
var i,llen,lResLen: integer;
    FChar,NChar: byte;
begin
  result:=pSrc;
  i:=0;
    ...
    begin
      ...
      if FChar >= $80 then
        begin
          ...
          if (NChar and $C0) <> $80 then Break;
          inc(lResLen);
          result[lResLen]:=Chr((FChar shl 6) or (NChar and $3F));
        end
      else
        begin
          inc(lResLen);
          result[lResLen]:=Chr(FChar);
        end;
    end;
  SetLength(result,lResLen);
end;

function ANSIToUTF8(const pSrc: String): String;
var i,lLen: integer;
    lUC: Word;
begin
  SetLength(result,Length(pSrc)*3);
  lLen:=0;
  for i:=1 to Length(pSrc) do
    begin
      lUC:=Ord(pSrc[i]);
      case lUC of
        $0000..$007F: begin
        ...
        $0080..$07FF: begin
          inc(lLen);
          result[lLen]:=Chr($C0 or (lUC shr 6));
          inc(lLen);
          result[lLen]:=Chr($80 or (lUC and $3F));
        end;
        $0800..$FFFF: begin
          inc(lLen);
          result[lLen]:=Chr($E0 or (lUC shr 12));
          inc(lLen);
          result[lLen]:=Chr($80 or ((lUC shr 6) and $3F));
          inc(lLen);
          result[lLen]:=Chr($80 or (lUC and $3F));
        end;
      end;
...
function utf8string(s: string): string;
begin
  Result := ANSIToUTF8(s);
end;
...
//convert Delphi 5 string to utf8
...
//TODO: missing conversion ANSI to UTF8
...
begin
  try
    SQLutf8 := ANSIToUTF8(SQL);
    if Sqlite3_Prepare_v2(self.fDB, PAnsiChar(SQLutf8), -1, Stmt, NextSQLStatement) <>
      SQLITE_OK then
...
begin
  ...
  if Sqlite3_Prepare(self.fDB, PAnsiChar(utf8sql), -1, Stmt, NextSQLStatement) <>
    SQLITE_OK then
    RaiseError('Error executing SQL', SQL)
  else
...
begin
  if Assigned(Query.Statement) then
  begin
    Query.SQL := ANSIToUTF8(Query.SQL);
    ...
    begin
      ...
      begin
        case par.valuetype of
          ...
          SQLITE_TEXT:
          begin
            par.valuedata := ANSIToUTF8(par.valuedata);
...
begin
  ...
  try
    ...
    if Sqlite3_Prepare_v2(DB.fDB, PAnsiChar(sqlutf8), -1, Stmt, NextSQLStatement) <> SQLITE_OK then
...
begin
  ...
  Result := fCols.IndexOf(ANSIToUTF8(AnsiUpperCase(FieldName)));
...
begin
  ...
  if (self.fResults[(self.frow * self.fColCount) + I] = nil) then
    ...
    Result := UTF8ToANSI(self.GetFields(I));
end;
...
begin
  ...
  self.fColCount := 0;
  self.fSQL := sqlutf8;
  if Sqlite3_Prepare_v2(DB.fDB, PAnsiChar(sqlutf8), -1, fStmt, NextSQLStatement) <> SQLITE_OK then
    DB.RaiseError('Error executing SQL', SQL);
  if (fStmt = nil) then
    DB.RaiseError('Could not prepare SQL statement', SQL);
  DB.DoQuery(SQL);
...
begin
  Result := UTF8ToANSI(fCols[I]);
end;

function TSQLiteUniTable.GetFieldByName(FieldName: string): string;
begin
  Result := UTF8ToANSI(GetFields(self.GetFieldIndex(FieldName)));
end;
...
begin
  Result := UTF8ToANSI(Sqlite3_ColumnText(fstmt, i));
end;
