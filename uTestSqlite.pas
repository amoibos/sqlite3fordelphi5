...
sSQL := 'INSERT INTO testtable(Name,OtherID,Number,Notes) VALUES ("Ölschlegel",12,4758.3265,"More notes");';
//do the insert
sldb.ExecSQL(sSQL);
...
if not FileExists(slDBPath + 'test.db') then begin
MessageDLg('Test.db does not exist. Click Test Sqlite 3 to create it.',mtInformation,[mbOK],0);
exit;
end;
...
try
 ...
 if sldb.Backup(sldbBak) = 101 then //SQLITE_DONE
  pnstatus.Caption := 'Backup successful'
  else
  pnstatus.Caption := 'Backup failed';
 finally
 sldbBak.Free;
 end;
